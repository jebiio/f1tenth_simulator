...
def stop_car():
    pub = rospy.Publisher('/new_drive', AckermannDriveStamped, queue_size=1)
    for i in range(50):
...
if __name__ == '__main__':
    ...
    pub_drive_param = rospy.Publisher('/new_drive', AckermannDriveStamped, queue_size=1)
    ...
    while not rospy.core.is_shutdown_requested():
        ...
        msg = AckermannDriveStamped()
        msg.header.stamp = rospy.Time.now()
        msg.drive.steering_angle = (steering_percentage) *(-0.34/100)
        print("steering_percentage : ", steering_percentage)
        msg.drive.speed = (torque_percentage) * 15/100
        print("torque_percentage : ", torque_percentage)
